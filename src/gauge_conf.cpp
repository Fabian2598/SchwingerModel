...
void GaugeConfV2::initialization() {
    int n;
...
//U_mv(x) = U_m(x) U_v(x+m) U*_m(x+v) U*_v(x)
//mu = 0 time direction, mu = 1 space direction
void GaugeConfV2::Compute_Plaquette01() {
	//U_01(n) = U_0(n) U_1(n+0) U*_0(n+1) U*_1(n)
...
//Compute staple at coordinate (x,t) in the mu-direction
void GaugeConfV2::Compute_Staple() {
    MPI_Status status;
    //U_v(x) U_m(x+v) U*_v(x+m) + U*_v(x-v) U_m(x-v) U_v(x+m-v)
    //mu = 0 time direction, mu = 1 space direction
    int n;
...
double GaugeConfV2::MeasureSp_HMC() {
	//Plaquettes have to be computed during the HMC update
    double local_Sp = 0.0;
    int n;
    //reduction over all lattice points and spin components
...
double GaugeConfV2::Compute_gaugeAction(const double& beta) {
	double local_action = 0.0;
    int n;
	for(int x = 1; x<=mpi::width_x; x++){
	    for(int t = 1; t<=mpi::width_t; t++){
...
void GaugeConfV2::SaveConf(const std::string& Name){
    using namespace LV;
    ...
    spinor_v2 GlobalConf((Nt+2)*(Nx+2)*2);
...
void GaugeConfV2::ReadConf(const std::string& Name){
    using namespace LV;
    std::ifstream infile(Name, std::ios::binary);
    if (!infile) {
       std::cerr << "File " << Name << " not found " << std::endl;
    ...
    spinor_v2 GlobalConf((Nt+2)*(Nx+2)*2); //Temporary variable to store the full configuration
