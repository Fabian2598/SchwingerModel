...
public:
	...
	GaugeConfV2() {
		...
		Conf = spinor_v2(mpi::maxSizeH); //Gauge configuration
		Staples = spinor_v2(mpi::maxSizeH); //Staples
	}

	/*
	Copy constructor
	*/
	GaugeConfV2(const GaugeConfV2& GConfig) {
		Conf = GConfig.Conf; 
		Staples = GConfig.Staples; 
		Plaquette01 = new c_double[mpi::sitesH];
	...
	Assignment operator
	*/
	GaugeConfV2& operator=(const GaugeConfV2& GConfig) {
		if (this != &GConfig) {
			Conf = GConfig.Conf;
			Staples = GConfig.Staples;
			delete[] Plaquette01;
