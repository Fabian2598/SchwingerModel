...
/*
    ...
    v: input, x: output
*/
spinor D_B_old(const c_matrix& U, const spinor& phi, const double& m0,const int& block);
...
/*
    ...
    phi: right-hand side, dim(phi) = 2 * sap_lattice_sites_per_block,
    phi_full: I_B_v(phi),
    x0: initial guess, 
    m0: mass parameter,
    m: restart length,
    ...
    output dimension = 2 * sap_lattice_sites_per_block
