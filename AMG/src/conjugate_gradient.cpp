...
c_matrix bi_cgstabV2(c_matrix (*func)(const c_matrix&, const c_matrix&, const double&), const int& dim1, const int& dim2,
const c_matrix& U, const c_matrix& phi, const c_matrix& x0, const double& m0, const int& max_iter, const double& tol, const bool& print_message) {
    ...
    //D_D_dagger_phi(U, phi, m0); //DD^dagger  
    c_matrix r(dim1, c_vector(dim2, 0));  //r[coordinate][spin] residual
    c_matrix r_tilde(dim1, c_vector(dim2, 0));  //r[coordinate][spin] residual
    c_matrix d(dim1, c_vector(dim2, 0)); //search direction
    c_matrix s(dim1, c_vector(dim2, 0));
    c_matrix t(dim1, c_vector(dim2, 0));
    c_matrix Ad(dim1, c_vector(dim2, 0)); //D*d
    c_matrix x(dim1, c_vector(dim2, 0)); //solution
    c_double alpha, beta, rho_i, omega, rho_i_2;;
    x = x0; //initial solution
